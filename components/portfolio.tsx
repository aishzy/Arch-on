...
function SectionHeading({ index, title, count }: { index: string; title: string; count?: string }) { return <motion.div className="section-heading" initial="hidden" whileInView="visible" viewport={{ once: true, amount: .45 }} variants={{ hidden: {}, visible: { transition: { staggerChildren: .08 } } }}><motion.span variants={headingItem}>{index}</motion.span><motion.h2 variants={headingItem}>{title}</motion.h2>{count && <motion.small variants={headingItem}>{count}</motion.small>}</motion.div>; }
